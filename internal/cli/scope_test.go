...
func TestServerRuntimeContextByScopeAndTransport(t *testing.T) {
	...
	workspaceContext, appErr := serverRuntimeContext(config.Server{Scope: config.ScopeWorkspace, Stdio: config.Stdio{Command: literalValue("server")}}, context)
	...
	globalStdio, appErr := serverRuntimeContext(config.Server{Scope: config.ScopeGlobal, Stdio: config.Stdio{Command: literalValue("server")}}, context)
	...
	globalHTTP, appErr := serverRuntimeContext(config.Server{Scope: config.ScopeGlobal, HTTP: &config.HTTP{Endpoint: literalValue("https://example.test")}}, invocationContext{ProjectRoot: workspace, GlobalRoot: filepath.Join(t.TempDir(), "missing")})
...
func TestGlobalStdioAndLSPRequireGlobalRootDirectory(t *testing.T) {
	...
	if _, appErr := serverRuntimeContext(config.Server{Scope: config.ScopeGlobal, Stdio: config.Stdio{Command: literalValue("server")}}, context); appErr == nil || appErr.code != "global_root_unavailable" {
